...
pub fn generate_target_ir(
    ...
    match provider {
        AiProvider::OpenAI { api_key, model } => {
            let (value, debug) = openai_generate(
                &api_key,
                &model,
                sculpt_ir,
                target_spec,
        ...
        AiProvider::Anthropic { api_key, model } => {
            let (value, debug) = anthropic_generate(
                &api_key,
                &model,
                sculpt_ir,
                target_spec,
        ...
        AiProvider::Gemini { api_key, model } => {
            let (value, debug) = gemini_generate(
                &api_key,
                &model,
                sculpt_ir,
                target_spec,
        ...
        AiProvider::Stub => {
            ...
            patch_target_ir_with_deterministic_parts(
                &mut value,
                &target_spec.standard_ir,
                sculpt_ir,
            );
            Ok((value, None))
        }
...
fn build_prompt(
    ...
    input.push_str(
        "Output must follow the compact schema exactly (positional arrays, no extra keys).\n",
    );
    ...
    input.push_str(
        "  action=\"modal.ok\" means show a simple OK modal when the button is clicked.\n\n",
    );
    if layout_required {
...
fn patch_target_ir_with_deterministic_parts(
    target: &mut Value,
    standard_ir: &str,
    sculpt_ir: &Value,
) {
    ...
    for state in states {
        ...
        for stmt in statements {
            if let Some(on) = stmt.get("On").and_then(Value::as_object) {
                ...
                if let (Some(event_obj), Some(dst)) = (event, target_state) {
                    let ev_name = event_obj
                        ...
                        .and_then(Value::as_str)
                        .unwrap_or_default();
                    let ev = normalize_event_name(
                        ev_name,
                        event_obj.get("args").and_then(Value::as_array),
                    );
                    if !ev.is_empty() {
            ...
            if let Some(expr) = stmt.get("Expr").and_then(Value::as_object) {
                let expr_name = expr.get("name").and_then(Value::as_str).unwrap_or_default();
                if expr_name == "render" {
                    if let Some(args) = expr.get("args").and_then(Value::as_array) {
                        if let Some(first) = args.first().and_then(Value::as_object) {
                            if let Some(call) = first
                                ...
                                .and_then(Value::as_object)
                            {
                                if let Some(item) = render_item_from_call(call) {
...
fn inject_runtime_rules(root: &mut serde_json::Map<String, Value>, sculpt_ir: &Value) {
    ...
    for rule in rules {
        ...
        let event = trigger
            ...
            .unwrap_or_default();
        let when = trigger.get("When").and_then(extract_when_condition);
