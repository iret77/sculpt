...
//
// To Run:
// 1. Install dependencies: npm install inquirer @types/inquirer
// 2. Execute with ts-node: ts-node ./incident-triage.ts
//
// Usage:
//   Interactive Mode: ./incident-triage.ts
//   Automation/JSON Mode: ./incident-triage.ts --simulate <1|2|3>
...
function runSimulation(planId: number): void {
  ...
  if (!plan) {
    console.error(`Error: Invalid plan ID "${planId}". Please use 1, 2, or 3.`);
    process.exit(1);
  }
...
/**
 * Handles the interactive CLI menu mode.
 */
async function runInteractive(): Promise<void> {
  try {
    const { selectedPlanName } = await inquirer.prompt([
      {
        type: 'list',
        name: 'selectedPlanName',
        message: 'What type of incident are you seeing?',
        choices: INCIDENT_PLANS.map((plan) => ({
    ...
    const selectedPlan = INCIDENT_PLANS.find((p) => p.name === selectedPlanName);

    if (selectedPlan) {
      displayPlan(selectedPlan);
    } else {
      console.error('An unexpected error occurred. Could not find the selected plan.');
    }
  } catch (error) {
    console.error('The interactive prompt failed to run.', error);
    process.exit(1);
  }
}
...
async function main(): Promise<void> {
  ...
  if (simulateIndex !== -1) {
    ...
    if (!planIdArg) {
      console.error('Error: The --simulate flag requires an argument (1, 2, or 3).');
      process.exit(1);
    }
